...
try:
    from pi_assistant.cli import logging as cli_logging_utils
except ImportError:
    ...
    class LoggingUtilsModule(ModuleType):
        ...
        def __init__(self) -> None:
            super().__init__("pi_assistant.cli.logging")
    ...
    sys.modules["pi_assistant.cli.logging"] = logging_utils
    cli_logging_utils = logging_utils

from pi_assistant.network.websocket.client import WebSocketClient
...
def verbose_capture(monkeypatch):
    ...
    monkeypatch.setattr(
        "pi_assistant.network.websocket.client.LOGGER.verbose",
        _capture,
    )
    return records
...
async def test_connect_waits_for_session(monkeypatch):
    ...
    monkeypatch.setattr("pi_assistant.network.websocket.client.websockets", dummy_module)
...
async def test_connect_warns_when_session_timeout(monkeypatch, capsys):
    ...
    monkeypatch.setattr("pi_assistant.network.websocket.client.websockets", dummy_module)
    monkeypatch.setattr("pi_assistant.network.websocket.client.asyncio.wait_for", fake_wait_for)
...
async def test_receive_events_handles_connection_closed(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "pi_assistant.network.websocket.client.websockets",
