...
def _require_model_factory() -> ModelFactory:
    wake_word_module = sys.modules.get("pi_assistant.audio.wake_word")
    candidate = None
    if wake_word_module is not None:
        candidate = getattr(wake_word_module, "Model", None)

    factory = candidate or Model or _MODEL_FACTORY
    if factory is None:
        raise RuntimeError(
