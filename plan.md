...
├── plan.md                  # This file
└── .venv/                   # Managed virtual environment (generated by uv)
```
...
1. Define dependencies in `pyproject.toml` (managed with uv)
...
- ✅ **uv adopted**: Dependency resolution, locking, and virtualenv management handled by `uv`.
- ✅ **pyproject.toml in place**: Runtime + dev dependencies live in `[project]` tables with locking via `uv.lock`.
- ⏭️ Optional next step: consolidate additional tool configuration (e.g., Ruff) into `pyproject.toml` if desired.
