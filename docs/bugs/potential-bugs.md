...
### 1. WebSocket operations have no timeouts
...
### 2. PreRollBuffer size calculation missing channels multiplier
...
### 4. Audio queue overflow silently drops frames
...
### 5. Wake word retriggering can bypass silence detection indefinitely
...
### 6. Stop command substring matching triggers on unrelated phrases
...
### 7. Linear resampler state not thread-safe
...
### 8. Confirmation cue errors not fully logged
...
### 9. No validation of audio chunk size before calculations
...
### 10. Response task cancellation without awaiting completion
...
### 11. WebSocket session timeout handling suboptimal
...
### 12. No bounds checking on audio format assumptions
...
### 13. Potential division by zero in chunk duration calculation
...
### 14. Missing error context in event receiver exception handling
...
### 15. Awaiting server stop without retry mechanism
