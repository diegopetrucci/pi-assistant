...
Context: Recent runs (see `logs/2025-11-17T22-41-04.473.log`) show ~14 s between `[ASSISTANT] Awaiting OpenAI response...` and `[ASSISTANT] Response received`. The pause happens entirely inside `LLMResponder.generate_reply` (`src/pi_assistant/assistant/llm.py`), so we need to make the LLM stage faster or feel faster.
...
- **Use a faster model** – The default model `gpt-5-mini-2025-08-07` prioritizes quality over latency, and paired with TTS+search it routinely crosses 10 seconds. Evaluate `gpt-4o-mini` or a Realtime-optimized sibling to trade a bit of quality for much lower response times.
