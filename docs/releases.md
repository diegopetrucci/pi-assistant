...
- Run the usual quality gates locally (`uv run ruff check .`, `uv run python -m unittest tests/test_wake_word.py`, and `uv run pytest -v`) and open a PR describing the release.
