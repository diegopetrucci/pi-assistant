...
* Versioning is tracked via the `[project].version` field in `pyproject.toml`.
* The repository is currently on `0.2.0`; bump this value in every release PR so the merge commit clearly identifies what will ship when the tag is created.

## Steps to Cut a Release
- Branch from `main` (example: `release/v0.1.1`) and bump the version in `pyproject.toml`.
- Run the full quality gates locally (`uv run pyright`, `uv run pytest -v`, `uv run ruff check .`, and `uv run ruff format .`) and open a PR describing the release.
