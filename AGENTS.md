...
* Write commits in the form `component: one-line summary` (e.g., `audio: harden portaudio fallback`) and keep bodies under 72 columns with “why” over “what”.
* Each PR should describe the user-facing behavior, link relevant issues, and include verification steps (`uv run pi-assistant`, `uv run pytest`).
* Attach screenshots or logs when the change impacts CLI output, note config additions in `README.md`, and seek a second review for hardware-touching changes.
...
* Whenever you add or change CLI flags, configuration keys, assistant behavior, or setup steps, mirror the change by updating the pointer in `README.md` and documenting the details in `docs/cli.md` (keep it as the canonical source), plus drop any long-form writeups in `docs/` as needed.
